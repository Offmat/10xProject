...
| `spec/fixtures/` | YAML fixtures when not using factories |
| `spec/support/` | shared helpers — auto-loaded via @spec/rails_helper.rb |

## Conventions

- Copy `describe`/`context`/`it` naming and matcher style from the nearest `_spec.rb` in the same folder. Run `bin/rubocop` on touched spec files.
