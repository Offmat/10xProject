...
|---|---|---|---|
| A | MVP path | `F-01` → `S-01` → `S-02` → `F-04` → `S-03` → `S-04` | Speed bias: must-have order through north star (`S-03`) then stats. `F-04` gated S-03 form work. |
...
| D | Agent form conventions | `F-04` | Research + durable guide/rules for interactive Rails forms; joined main path before finishing S-03 form work. |
| E | System-spec / E2E floor | `F-05` | Capybara + Cuprite, fidelity seed, CI Chrome, `/10x-e2e-capybara`; quality floor **after** S-03 shipped (needs the real form to drive). |
...
- **Unlocks:** test-plan Phase 1 (system half) + Phase 4 CI floor; safer future Stimulus/form changes; regression net under S-04 and beyond
...
_No cross-cutting roadmap questions. PRD `## Open Questions` were resolved 2026-05-21 (login mechanism, friend-circle policy); catalog seeding re-resolved 2026-06-05 (import service, console invocation); catalog provider for MVP set to Wikidata 2026-06-07 (switch/evaluation → P-07). F-02 data access switched from Action API (`wbgetentities`) to SPARQL Query Service 2026-06-08._
...
- **Admin account / catalog curation** — Why parked: PRD Non-Goals; flat users only in MVP. F-02 delivers the import service; P-03 adds admin auth + UI around it. **Note (D1):** before scaling to large imports, add `GROUP BY` + `SAMPLE()` to the SPARQL query to prevent non-deterministic row selection when a game has multiple truthy values for the same property; also switch from GET to POST for long queries (D4).
- **Push notifications** — Why parked: PRD Non-Goals; in-app inbox only in v1.
- **Unfriend / remove friend** — Why parked: deferred from S-02 (mutual-friend-circle) to keep the slice focused on send/accept/decline/cancel; PRD does not require it. Revisit alongside session-tagging implications in or after S-03.
